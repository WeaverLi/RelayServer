...
function FileInfo() {
  ...
  //@ 76byte 命令索引表头
  this.cmdNum = 0;                  //2字节,对于空调要重新统计，其他的直接读取,这个是算出来的不需要写在文件里面
  this.indexAreaSize = 0;           //2字节,索引表的单位个数，512或0：0x0002
  this.idxSize = 0;                 //1字节,索引表项大小，2或0
  this.cmdHeadSize = 0;             //1字节,命令头大小，2或32： 0x20
  //@ 80byte
  this.cmdSize = 0;                 //2字节，命令项大小（一般为160+32:0xc000，空调为160+2:0xa200， 对于要生成码的也用192），每个命令项大小相同，用于连续计算命令码的偏移，码实际长度可以不同。
  this.fileSize = 0;                //4字节，这个也是算出来的，不用写在文件里面
  // @ 82byte
}
